...
// We don't have the password or email info stored in sessions so instead we can get the results from the database.
$stmt = $con->prepare('SELECT * FROM top_5_wasted');
// In this case we can use the account ID to get the account info.
$stmt->bind_param('i', $_SESSION['wasted_id_seed']);
$stmt->execute();
$stmt->bind_result($wasted_id_seed, $wasted_name_seed, $amount_seed_wasted);
$stmt->fetch();
$stmt->close();
...
if(isset($_POST['new']) && $_POST['new']==1) {
	...
	$status = "Wasted Quantity Successfully.";
...
$status = "";
if(isset($_POST['new']) && $_POST['new']==1) {
	$wasted_id_seed=$_REQUEST['wasted_id_seed'];
	$amount_seed_wasted=$_REQUEST['amount_seed_wasted'];
	$quantity=$_REQUEST['quantity'];
	$update = "UPDATE top_5_wasted SET amount_seed_wasted = amount_seed_wasted + '".$quantity."' where wasted_id_seed='".$seed_number."'";
...
<html>
	...
	<body class="loggedin">
		...
		<div class="content">
			<h2>Remove Wasted Page</h2>
			<div>
				<p>Remove Wasted Quantity:</p>
